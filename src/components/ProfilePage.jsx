  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="glass-panel relative mb-8 overflow-hidden p-8"
        style={{ backgroundColor: 'rgba(34, 36, 42, 0.92)', borderColor: 'rgba(59, 130, 246, 0.2)' }}
      >
        <h4 className="relative z-10 mb-4 text-lg font-black uppercase tracking-widest text-white">
          Реферальная ссылка
        </h4>
        <p className="relative z-10 mb-6 text-sm text-white/50">
          Приглашай друзей и забирай награду за задание.
        </p>

        <motion.button
          whileTap={{ scale: 0.98 }}
          onClick={handleCopyReferral}
          className="relative z-10 w-full rounded-2xl py-4 text-sm font-black uppercase tracking-widest"
          style={{ backgroundColor: 'white', color: PAGE_BG }}
        >
          Пригласить друзей
        </motion.button>
      </motion.div>
      ...
      <motion.div
        ...
        style={{ backgroundColor: 'rgba(34, 36, 42, 0.92)', borderColor: 'rgba(255, 255, 255, 0.1)' }}
      >
        <h4 className="mb-6 text-lg font-black uppercase tracking-widest text-white">Задания</h4>

        {!tasks.length ? (
          <div className="py-8 text-center">
            <p className="text-sm text-white/30">Все задания выполнены</p>
          </div>
        ) : (
          <div className="space-y-3">
            {tasks.map((task) => (
              <div key={task.id} className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-bold text-white">{task.title}</p>
                  <div className="mt-1 flex items-center gap-1.5 text-xs font-bold text-yellow-400">
                    <img
