...
export default function UpgradeGame({ isPage, inventory, setInventory, balance, setBalance, setSpent }) {
  ...
  const handleAnimationComplete = () => {
    ...
    if (res.success) {
      ...
      if (res.consolation) {
         setConsolation(res.consolation.item);
         setInventory(prev => [{...res.consolation.item, id: Date.now()}, ...prev]);
         if (window.Telegram?.WebApp) {
           window.Telegram.WebApp.showAlert("Не повезло! Но мы ценим тебя, держи Утешительный Кейс!");
         }
      }
    ...
    return (
      <div className="relative w-48 h-48 mx-auto mt-6">
        ...
        <svg viewBox="0 0 200 200" className="w-full h-full rotate-[-90deg]">
          ...
          {successChance < 100 && (
            <path d={describeArc(100, 100, radius, successChance, 100)} fill="none" stroke="rgba(107, 114, 128, 0.4)" strokeWidth="10" strokeLinecap="round" />
          )}
          {successChance > 0 && (
            <path d={describeArc(100, 100, radius, 0, successChance)} fill="none" stroke="rgba(34, 197, 94, 0.6)" strokeWidth="12" strokeLinecap="round" className="filter drop-shadow-[0_0_8px_rgba(34,197,94,0.5)]" />
          )}
        </svg>
  ...
  const content = (
    <div className={`${isPage ? 'p-4 min-h-full pb-24' : ''}`}>
      <div className={`${!isPage ? 'glass-panel p-6 w-full max-w-lg mx-auto' : ''}`}>
        {!isPage && ( <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black text-white font-rounded uppercase tracking-widest">Апгрейд</h2></div> )}
        {isPage && ( <div className="mb-6 text-center"><h2 className="text-3xl font-black text-white font-rounded uppercase tracking-widest text-glow">Апгрейд</h2><p className="text-white/40 text-[10px] uppercase font-black tracking-[0.2em] mt-2">Улучшай свои предметы</p></div> )}

        {!inventory || inventory.length === 0 ? (
          <div className="glass-panel p-16 text-center border-white/5 bg-white/[0.02]"><p className="text-white/20 font-black text-xs uppercase tracking-[0.3em]">Инвентарь пуст</p></div>
        ) : (
          <div className="space-y-6">
                ...
                <div className="flex-1 text-center">
                  ...
                  {!selectedSlot1 ? ( <div className="w-32 h-32 mx-auto rounded-3xl border-2 border-dashed border-white/10 bg-white/[0.01] flex items-center justify-center"><span className="text-white/10 text-4xl font-black">?</span></div> ) : (
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="p-5 rounded-3xl bg-white/5 border border-white/10 relative shadow-2xl">
                      ...
                      <div className="flex items-center justify-center gap-1.5 text-xs text-white/70 font-black font-rounded mt-4 bg-black/40 py-1.5 rounded-xl">{selectedSlot1?.price || selectedSlot1?.cost || 0} <img src="/asset/Icons/TelegramStar.png" className="h-4 w-4" alt="Stars" onError={(e) => { e.currentTarget.src = '/asset/Gifts/Case.webp'; }} /></div>
                    </motion.div>
                  )}
                </div>
            ...
            {selectedSlot1 && selectedSlot2 && ( <UpgradeButtonContent canUpgrade={balance >= upgradeCost} isUpgrading={isUpgrading} upgradeCost={upgradeCost} handleUpgrade={handleUpgrade} /> )}
                ...
                <div className="grid grid-cols-3 gap-4 overflow-y-auto pr-2 pb-6 custom-scrollbar">
                  {eligibleTargets.map((gift, idx) => (
                    <motion.div key={idx} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { setSelectedSlot2(gift); setShowGiftSelector(false); triggerHaptic('light'); }} className={`p-4 rounded-3xl border-2 cursor-pointer transition-all flex flex-col items-center ${ selectedSlot2?.name === gift.name ? 'border-green-500/60 bg-green-500/10' : 'border-white/5 bg-white/[0.03] hover:border-white/10' }`}>
                      ...
                      <div className="flex items-center justify-center gap-1 text-green-400 text-[11px] font-black font-rounded bg-green-500/5 px-2 py-1 rounded-lg">{gift?.price || gift?.cost || 0} <img src="/asset/Icons/TelegramStar.png" className="h-3 w-3" alt="Stars" onError={(e) => { e.currentTarget.src = '/asset/Gifts/Case.webp'; }} /></div>
                    </motion.div>
                  ))}
  ...
  return ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">{content}</motion.div> );
}

function UpgradeButtonContent({ canUpgrade, isUpgrading, upgradeCost, handleUpgrade }) {
  if (isUpgrading) { return ( <div className="w-full py-6 rounded-2xl font-black text-2xl flex items-center justify-center gap-4 bg-green-500/10 border-2 border-green-500/40 text-green-400 font-rounded uppercase tracking-tight shadow-[0_0_30px_rgba(34,197,94,0.2)]"><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }} className="w-7 h-7 border-4 border-green-400/20 border-t-green-400 rounded-full" />Апгрейдим...</div> ); }
  return (
    <div className="w-full space-y-4">
      <motion.button whileHover={{ scale: canUpgrade ? 1.02 : 1 }} whileTap={{ scale: canUpgrade ? 0.98 : 1 }} onClick={() => { handleUpgrade(); }} disabled={isUpgrading || !canUpgrade} className="w-full py-6 rounded-2xl font-black text-2xl disabled:opacity-50 transition-all flex items-center justify-center gap-3 uppercase tracking-tighter shadow-2xl relative overflow-hidden group" style={{ backgroundColor: canUpgrade ? 'rgba(34, 197, 94, 0.3)' : 'rgba(255, 255, 255, 0.02)', border: `2px solid ${canUpgrade ? 'rgba(34, 197, 94, 0.6)' : 'rgba(255, 255, 255, 0.05)'}`, color: canUpgrade ? '#4ade80' : 'rgba(255,255,255,0.1)' }}>
        ...
        Апгрейд ({upgradeCost} <img src="/asset/Icons/TelegramStar.png" className="h-8 w-8" alt="Stars" onError={(e) => { e.currentTarget.src = '/asset/Gifts/Case.webp'; }} />)
      </motion.button>
